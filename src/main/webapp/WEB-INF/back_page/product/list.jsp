...
function isShow(pageNo,name,brandId,isShow) {
	...
	$("#jvForm").attr("action","isShow.do" + "?pageNo="  +  pageNo  +"&brandId=" + brandId + "&isShow=" + isShow + "&name=" + name);
	$("#jvForm").submit();
}
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<c:forEach items="${pagination.list }" var="entry">
		<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
			...
			<td align="center"><c:if test="${entry.isNew  ==0 }">否</c:if><c:if test="${entry.isNew  ==1}">是</c:if></td>
			<td align="center"><c:if test="${entry.isHot ==0 }">否</c:if><c:if test="${entry.isHot ==1}">是</c:if></td>
			<td align="center"><c:if test="${entry.isCommend ==0 }">否</c:if><c:if test="${entry.isCommend ==1}">是</c:if></td>
			<td align="center"><c:if test="${isShow==0 }">下架</c:if><c:if test="${isShow==1 }">上架</c:if></td>
			<td align="center">
			<a href="#" class="pn-opt">查看</a> | <a href="#" class="pn-opt">修改</a> | <a href="#" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a> | <a href="/sku/toList.do?productId=${entry.id}&pno=${entry.no}" class="pn-opt">库存</a>
			</td>
		</tr>
	</c:forEach>
	</tbody>
...
<input class="del-button" type="button" value="上架 " onclick="isShow('${pagination.pageNo}','${name }','${brandId }','${isShow }');"/>
