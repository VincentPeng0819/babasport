...
function myOrder(){
	window.location.href = "/buyer/index.shtml?returnUrl=";
}
</script>
</head>
<body>
<div class="bar"><div class="bar_w">
	...
	<ul class="r uls">
	<li class="dev">您好,欢迎 ${name} 来到新巴巴运动网！</li>
	<c:if test="${!isLogin }">
		...
		<h2 class="h2 h2_filter mt"><em title="商品筛选">商品筛选</em><cite><a href="javascript:void(0);" id="filterRest" title="重置筛选条件">重置筛选条件</a></cite></h2>
			...
			<c:if test="${flag}">
				...
				<c:if test="${brandName != null }">
					...
					<em>品牌：</em>${brandName }
					<cite title="关闭此筛选条件" onclick="window.location.href='/product/display/list.shtml?typeName=' + '${typeName }'+ '&typeId=' + '${typeId }' ">X</cite></a>	
				</c:if>
				
				<c:if test="${typeName != null }">
					<a href="javascript:void(0);">
					<em>类型：</em>${typeName }
					<cite title="关闭此筛选条件" onclick="window.location.href='/product/display/list.shtml?brandName=' + '${brandName  }'+'&brandId=' + '${brandId}' ">X</cite></a>
				</c:if>
			...
			<c:if test="${flag == false || brandName == null}">
				<li><b>品牌：</b><p>
					<a href="javascript:void(0);" title="不限" class="here">不限</a>
					<c:forEach items="${brands}" var="brand">
						<a href="javascript:void(0);" title="${brand.name }" onclick="window.location.href='/product/display/list.shtml?brandId=' + ${brand.id} + '&brandName=' + '${brand.name}' + '&typeName=' + '${typeName }' + '&typeId=' + '${typeId }' ">${brand.name }</a>
					</c:forEach>
					</p></li>
			</c:if>
				<li><b>价格：</b><p>
				...
				<c:if test="${typeName == null || flag == false}">
				<li><b>类型：</b>
				<p>
					<a href="javascript:void(0);" title="不限" class="here">不限</a>
					<c:forEach items="${ types}" var="type">
						<a href="javascript:void(0);" title="${type.name }" onclick="window.location.href='/product/display/list.shtml?typeName=' + '${type.name }' +'&typeId=' + ${type.id }  + '&brandName=' + '${brandName  }' + '&brandId=' + '${brandId}' ">${type.name }</a>
					</c:forEach>
				</p></li>
				</c:if>
				<li><b>材质：</b><p>
			...
			<div class="mt ofc">
				...
				<c:forEach items="${pagination.list }" var="entry">
					<li>
						...
						<dl>
							<!-- dt 10个文字+... -->
							<dt><a href="javascript:void(0)" onclick="window.open('/product/display/detail.shtml?productId='+'${entry.id}')" title="${entry.name }" >${entry.name}</a></dt>
							<!-- dt 25个文字+... -->
							<dd class="h40">${entry.description }</dd>
			...
			<div class="page pb15">
				<span class="r inb_a page_b">
					<c:forEach items="${pagination.pageView }" var="view">
						${view }
					</c:forEach>
				</span>
			</div>
