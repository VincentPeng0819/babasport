...
//结算
function trueBuy(){
 	window.location.href = "/shopping/trueBuy.shtml";
}
			...
			<c:forEach items="${buyCart.items }" var="item">
				<tr>
					<td class="nwp pic">
						<ul class="uls">
							<li>
								<a class="pic" title=" ${item.sku.product.name}" href="#"><img alt=" ${item.sku.product.name}" src="${item.sku.product.img.allUrl }"></a>
								<dl>
									<dt><a title=" ${item.sku.product.name}" href="#">  ${item.sku.product.name}-- ${item.sku.color.name}-- ${item.sku.size}</a></dt>
